          ...
          <RevealGroup className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((service, i) => (
              <RevealItem key={service.slug}>
                {/* The card surface is set here rather than on LiftCard: the
                    home page composes LiftCard with its own surfaces, so giving
                    the wrapper a default border would double it up there. */}
                <LiftCard className="flex h-full flex-col rounded-2xl border border-border bg-card p-7 shadow-[var(--shadow-soft)]">
