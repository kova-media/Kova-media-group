        ...
        if (node.type === 'link') {
          ...
          return (
            <a
              key={index}
              href={node.href}
              className="underline decoration-ink-300 underline-offset-4 transition-colors hover:decoration-ink-900"
...
function Block({ node }: { node: BlockNode }) {
  switch (node.type) {
    case 'heading': {
      ...
      return (
        <Tag className={cn('mt-10 mb-3 font-medium text-ink-950 first:mt-0', size)}>
          <Inline nodes={node.children} />
        </Tag>
      )
    }
    ...
    case 'list': {
      ...
      return (
        <Tag
          className={cn(
            'my-4 flex flex-col gap-2 pl-5 text-ink-700',
    ...
    case 'blockquote':
      return (
        <blockquote className="my-6 border-l-2 border-accent-500 pl-5 text-lg text-ink-800">
          <Inline nodes={node.children} />
        </blockquote>
      )
    ...
    default:
      return (
        <p className="my-4 text-ink-700 first:mt-0 last:mb-0">
          <Inline nodes={node.children} />
        </p>
      )
  }
